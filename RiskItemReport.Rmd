...
```{r, milestoneplot, echo=FALSE, warning=FALSE}
mockdf<- data.frame(ID = c("Project Initiation","Alternatives","Tentatively Selected Plan","Agency Decision","Senior Leaders Briefing","Chief's Report/Director's Report"),
                    yval = c(0.25,0.25,0.25,0.25,0.25,0.25),
                    xval = c(1,2,3,4,5,6))
mockdf$xwrap = str_wrap(mockdf$ID, width=10)

ggplot(mockdf, aes(x=ID, y=yval))+
  geom_segment(x=1, y = 0.25, xend = 6, yend = 0.25, size=1.5, color = "grey")+
  geom_point(size=10,color="grey", fill="snow1", shape=21, stroke=1.5)+
  geom_point(aes(x=risk_item$MILESTONE, y=0.25), colour="deepskyblue2", fill="deepskyblue2", shape = 21, size =10, stroke=1.5)+
  geom_text(aes(y=0.25,label=xwrap), vjust=2)+
