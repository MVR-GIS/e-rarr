...
library(readr)
library(dplyr)
erisk_item <- readr::read_csv("./inst/app/data/RISKLIST_FULL.csv",show_col_types = FALSE)
risk_item_db<-data.frame(erisk_item)
erisk_project <- readr::read_csv("./inst/app/data/PROJECTLIST_FULL.csv",show_col_types = FALSE)
...
riskpies <- risk_item_db |>
  dplyr::select("P2_NUMBER", "RISK_IDENTIFIER","PROJECT_NAME","RISK_NAME","RISKCATEGORY",
...
app_server <- function(input, output, session) {
  ...
  output$reportrend <- renderUI({
    ...
    withProgress(message = 'Rendering',{
      includeMarkdown(
        rmarkdown::render("./inst/app/rmd/RiskItemReport.Rmd", params=list(projID = input$projectInput, riskID = input$riskInput, p2ID = input$P2Input,shinyrend = TRUE)
        )
      )})
  })
  output$ProjRend <- renderUI({
    ...
    withProgress(message = 'Rendering',{
      includeMarkdown(
        rmarkdown::render("./inst/app/rmd/ProjectAllRiskReport.Rmd", params=list(projID = input$projectInput, p2ID = input$P2Input,shinyrend = TRUE))
      )
    })
  })
  output$AllRiskRend <- renderUI({
    ...
    withProgress(message = 'Rendering',{
      includeMarkdown(
        rmarkdown::render("./inst/app/rmd/AllRiskDetailTable.Rmd", params=list(projID = input$projectInput,p2ID = input$P2Input,shinyrend = TRUE)
        )
      )
    })
  })
  ...
  output$Top4s <-renderUI({
    ...
    withProgress(message = 'Rendering',{
      includeMarkdown(
        rmarkdown::render("./inst/app/rmd/ProjectTop4s.Rmd", params=list(projID = input$projectInput,p2ID = input$P2Input,shinyrend = TRUE)
        ))})})
  ...
  output$report <- downloadHandler(
    ...
    content = function(file) {
      rmarkdown::render(paste0("./inst/app/rmd/",filnm(),".Rmd"), output_file = file,
