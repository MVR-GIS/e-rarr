...
library(leaflet)
library(DT)
erisk_item <- read_csv("C:/workspace/e-rarr/data/RiskItem_FullView_Update.csv")
risk_item_db<-data.frame(erisk_item)
erisk_project <- read_csv("C:/workspace/e-rarr/data/RiskProject_FullView.csv")
risk_project_db<-data.frame(erisk_project)
risk_treat <- read_csv("C:/workspace/e-rarr/data/RiskTreatment_FullView.csv")
imagepath<-"C:/workspace/e-rarr/images/check.png"

```

Choose a project and risk item
```{r echo=FALSE}
selectInput("Project","Projects", choices=c(risk_treat$PROJECT_NAME))
...
risk_item<-filter(RiskImpactTable, RiskImpactTable$RISK_ID == params$riskID, RiskImpactTable$PROJECT_ID == input$project)
risk_treatment<-filter(risk_treat, risk_treat$PROJECT_ID == input$project, risk_treat$RISK_ID == params$riskID)
...
kbl(risk_treat_table,"html",col.names = NULL, align= "llccc") %>%
...
risk_transact <- read_csv("C:/workspace/e-rarr/data/RiskTransaction_FullView.csv")
...
risk_log_table[1:7]%>%
kbl("html", col.names =c("Edit Date", "Reason for Change", "Risk Log Note",  "Measures Implemented", "Cost","Schedule","Performance"), align="lllcccc") %>%
