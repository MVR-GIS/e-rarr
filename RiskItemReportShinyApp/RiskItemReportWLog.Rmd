...
risk_treat <- read_csv("C:/workspace/e-rarr//RiskItemReportShinyApp/data/RiskTreatment_FullView.csv",col_names = TRUE)
...
risk_transact<-filter(risk_transact, risk_transact$PROJECT_NAME == params$projID, risk_transact$RISK_NAME == params$riskID)
...
ggplot(mockdf, aes(x=factor(ID, level=c(level_order)), y=yval))+
  ...
  geom_point(size=8,color="grey", fill="snow1", shape=21, stroke=1.5)+
   geom_point(aes(x=risk_item$MILESTONE, y=0.15), colour="deepskyblue", fill="deepskyblue", shape = 21, size =8, stroke=1.5)+
...
``` {r risk log, echo = FALSE, warning=FALSE, eval= treat_condit}
