...
params:
  riskID: Arsenic Mobilization
  projID: Lake Okeechobee Watershed
  p2ID: ""
  runtime: shiny
---
...
library(DT)
options(scipen = 999)
setwd("C:/workspace/e-rarr/RiskItemReportShinyApp")
source("milestoneplot.R")
...
checkimagepath<-"C:/workspace/e-rarr/RiskItemReportShinyApp/www/check.png"
erisk_item <- read.csv("C:/workspace/e-rarr/RiskItemReportShinyApp/data/RISKLIST_FULL.csv")
risk_item_db<-data.frame(erisk_item)
...
figcondit <- ifelse(nrow(risk_item)>0,TRUE,FALSE)    
```
...
```{r milestoneplot, fig.align='center', fig.height=2, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE, eval=figcondit}
...
`r if(!figcondit){"Risk treatments have not been completed for this risk item"}`

```{r echo=F,fig.align= "center", fig.height=2, fig.width=8, eval = figcondit}
