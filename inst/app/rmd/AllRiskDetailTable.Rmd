...
library(formattable)
options(scipen = 999)
erisk_item <- read_csv("../data/erisk_item.csv")
risk_item_db<-data.frame(erisk_item)
erisk_project <- read_csv("../data/erisk_project.csv", 
...
#  mutate(COST_MEAN = COST_MEAN|> replace_na())

```

```{r echo=FALSE, eval = params$projID != ""}
Risk_List_Proj <-filter(RiskDeets, RiskDeets$PROJECT_NAME == params$projID)
Risk_List_Proj$COST_MEAN <- 
currency(Risk_List_Proj$COST_MEAN, format = "d")
...
Risk_List_Proj$COST_MEAN <- currency(Risk_List_Proj$COST_MEAN,format = "d")
header2<-if (params$p2sub != ""){
  paste("SubID:", params$p2sub)}
```
...
RiskDeetsTable<-Risk_List_Proj |>
  select("RISK_IDENTIFIER","RISK_NAME","RISKCATEGORY","RISK_STATEMENT" ,"LIFECYCLEPHASENAME","MILESTONE","LIKELIHOOD","DISCIPLINE",
    ...
    rename("RISK" = RISK_IDENTIFIER,
         ...
         "COST IMPACT" = COST_MEAN,
         "SCHEDULE IMPACT (DAYS)" = SCHEDULE_MEAN)
