...
<center>
```{r milestoneplot, fig.height=1.40, fig.width=8, echo=FALSE, message=FALSE, warning = FALSE}
...
monte_pie<-plot_ly() |>
  ...
  layout(annotations = list(
      ...
      text = c("Cost",paste("Mean:",
                            formattable::currency(round(risk_model$COST_MEAN,-3),digits = 0)))))|>
  add_pie(
    monte_pies,
...
risk_treat_table<-risk_treatment |>
  ...
  mutate(IMPLEMENT_IMG = case_when(
    IMPLEMENTED == 1 ~  paste0('<span style="color: green;">', bsicons::bs_icon("check-lg"), '</span>'),
    IMPLEMENTED == 0 ~ paste0('<span style="color: red;">', bsicons::bs_icon("x-lg"), '</span>'))) |>
  select(RISK_TREATMENT_ID,PROPOSED_RISK_TREATMENT,COST_IMPACT_RISK_TREATMENT,
         SCHEDULE_IMPACT_RISK_TREATMENT, IMPLEMENT_IMG) |>
  mutate(PROPOSED_RISK_TREATMENT = paste0(( "Measure "), 
                                          stringr::str_to_title(english::english(
                                            RISK_TREATMENT_ID)),": ",
...
mutate(across(c(COST_RISK_RANKING, SCHEDULE_RISK_RANKING, PERFORMANCEIMPACT_RISK_RANKING), 
                ~ case_when(
                  ...
                  . == 4 ~ '#E31A1C'),
                .names = "{.col}color")) |>
  mutate(TREAT_IMPLEMENT_IMG = if_else(is.na(TREATMENTS_IMPLEMENTED),paste0('<span style="color: red;">', bsicons::bs_icon("x-lg"), '</span>'),  paste0('<span style="color: green;">', bsicons::bs_icon("check-lg"), '</span>')))
...
risk_log_table[1:10] |>
  ...
  kbl(format = "html", escape = FALSE, 
      col.names = c("Edit Date", "Reason for Change", "Risk Log Note", "Measures Implemented", 
                     "Cost Ranking", "Schedule Ranking", "Performance Ranking"), 
      align = "lllcccc") |>
