...
risk_transact <- utils::read.csv("../data/risk_transact.csv")
risk_transact <- data.frame(risk_transact)
milestonedf <- utils::read.csv("../data/milestonedf.csv")
milestone_df <-data.frame(erarr::milestonedf)
risk_modeled <-utils::read.csv("../data/erisk_modeled.csv")
...
erisk_ItemProj <- left_join(risk_item_db, risk_project_db, 
                            by = c("PROJECT_ID"), relationship = "many-to-many") 
...
RiskImpactTable <- erisk_ItemProj |>
  select("RISK_ID", "RISK_NAME_ID", "RISK_NAME", "RISK_IDENTIFIER", 
         "P2_NUMBER.x", "PROJECT_ID", "PROJECT_NAME.x", "RISK_NAME", 
         "RISKCATEGORY", "DISCIPLINE", "RISK_STATEMENT", "LIKELIHOOD_DESC", 
         "PROB_OCCURRENCE_EVIDENCE", "COST_IMPACT_LOWEST", 
         "COST_IMPACT_MOSTLIKELY", "COST_IMPACT_HIGHEST", 
         ...
         "PERFORMANCEIMPACT_DESC", "PERFORMANCEIMPACT", 
         "PERFORMANCE_IMPACT_DESC", "PROJECTPHASEID", "MILESTONE", 
         "RISK_MANAGER", "COST_IMPACT_DISTTYPE", "SCHEDULE_IMPACT_DISTTYPE", 
         "NO_PERFORMANCE_IMPACT", "NO_COST_IMPACT", "NO_SCHEDULE_IMPACT")

risk_transact <- left_join(risk_transact, risk_item_filt, 
                           by = c("PROJECT_ID", "RISK_ID", "PROJECT_NAME"), 
                           relationship = "many-to-many")     
risk_treat <- left_join(risk_treat, risk_item_filt, 
                        by = c("PROJECT_ID", "RISK_ID", "PROJECT_NAME"), 
                        relationship = "many-to-many")
```

```{r echo=FALSE, eval = params$projID != ""}
risk_item <- filter(RiskImpactTable, 
                    RiskImpactTable$RISK_NAME_ID == params$riskID, 
                    RiskImpactTable$PROJECT_NAME.x == params$projID)
risk_treatment <- filter(risk_treat, 
                         risk_treat$PROJECT_NAME == params$projID, 
                         risk_treat$RISK_NAME_ID == params$riskID)
risk_model <- filter(risk_modeled, 
                     risk_modeled$PROJECT_ID == risk_item$PROJECT_ID, 
                     risk_modeled$RISK_ID == risk_item$RISK_ID)
```
...
risk_treat_table <- risk_treatment |>
  ...
  mutate(IMPLEMENT_IMG = case_when(IMPLEMENTED == 1 ~ checkimagepath, 
                                   IMPLEMENTED == 0 ~ " ")) |>
  select(RISK_TREATMENT_ID, PROPOSED_RISK_TREATMENT, COST_IMPACT_RISK_TREATMENT, 
  ...
  mutate(SCHEDULE_IMPACT_RISK_TREATMENT = paste(SCHEDULE_IMPACT_RISK_TREATMENT, 
                                                "Days"))
...
header <- paste("Project:", risk_item$PROJECT_NAME.x, 
                " ", (risk_item$RISK_IDENTIFIER))
cat("####", header)
```
...
<center>
```{r proj montepies, message=FALSE, warning=FALSE, echo=FALSE,fig.height=1.75, fig.width=7, eval=monte_condit}
monteframe <- data.frame(risk_model)
ranking = c("Opportunity", "Low", "Medium", "High")
colors = c('rgb(31,120,180)', 'rgb(51,160,44)', 'rgb(255,127,0)', 
           'rgb(227,26,28)')
cost_values = c(monteframe$COST_PROBOPP, monteframe$COST_PROBLOW, 
                monteframe$COST_PROBMOD, monteframe$COST_PROBHIGH)
schedule_values= c(monteframe$SCHEDULE_PROBOPP, monteframe$SCHEDULE_PROBLOW, 
                   monteframe$SCHEDULE_PROBMOD, monteframe$SCHEDULE_PROBHIGH)
performance_values = c(monteframe$OUTCOME_PROBOPP, monteframe$OUTCOME_PROBLOW, 
                       monteframe$OUTCOME_PROBMOD, monteframe$OUTCOME_PROBHIGH)
monte_pies = data.frame(ranking, colors, 
                        round(cost_values, 2), 
                        round(schedule_values, 2), 
                        round(performance_values, 2))
names(monte_pies) = c("ranking", "colors", "cost_values", "schedule_values", 
                      "performance_values")
...
monte_pie <- plot_ly() |>
  ...
  layout(annotations = list(
    x = c(0.12, 0.10), y = c(1.2, -0.15), 
    xref = "paper", yref = "paper", 
    showarrow = F, 
    text = c("Cost", 
             paste("Mean:", formattable::currency(monteframe$COST_MEAN, 
                                                  digits = 0))))
  ) |>
  add_pie(
    monte_pies,
  ...
  layout(annotations = list(
    ...
    showarrow = F,
    text = c("Schedule", 
             paste("Mean:", monteframe$SCHEDULE_MEAN, "Days")))
  ) |>
  add_pie(
    monte_pies,
  ...
  pies <- monte_pie |>
    plotly::layout(title = "",
                  ...
                  showlegend = F)
      #           annotations = list(
      # x = c(0.10, 0.49, 0.9, 0.12, 0.485),
      # y = c(1.2, 1.2, 1.2, -0.15,-0.15),
      # xref = "paper",
      # yref = "paper",
      # showarrow = F,
      # text = c("Cost", "Schedule", "Performance", 
      #          paste("Mean:", 
      #                formattable::currency(monteframe$COST_MEAN, 
      #                                      digits = 0)),
      #          paste("Mean:", monteframe$SCHEDULE_MEAN,"Days"))))
...
risk_item_table <- risk_item |>
  ...
  mutate(
    ...
    COST_IMPACT_MOSTLIKELY = paste0("<p style = 
                                    'font-family: Arial Narrow;font-size:8pt'>",
                                    "Most Likely","</p>", 
                                    currency(COST_IMPACT_MOSTLIKELY)),
