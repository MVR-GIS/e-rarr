...
params:
  riskID: Impacts due to Uncertain Bathymetric Data
...
library(erarr)
options(scipen = 999)
erisk_item <- readr::read_csv("../data/RISKLIST_FULL_0320245.csv", 
                              col_types=cols(P2_SUB_IDENTIFIER =  col_double()))
risk_item_db<-data.frame(erisk_item)
erisk_project <- readr::read_csv("../data/PROJECTLIST_FULL_03226024.csv", 
                                 col_types=cols(P2_SUB_IDENTIFIER =  col_double()))
risk_project_db<-data.frame(erisk_project)
risk_treat <- readr::read_csv("../data/RISKTREATMENTLIST_FULL_032024.csv",
                              col_names = TRUE, col_types=cols(P2_SUB_IDENTIFIER =  col_double()))
...
erisk_ItemProj<-left_join(risk_item_db, risk_project_db, by =c("PROJECT_ID"))                                                                                                                          
risk_item_filt<- risk_item_db |>
  select(RISK_ID, PROJECT_ID, RISK_NAME , PROJECT_NAME, P2_NUMBER, P2_SUB_IDENTIFIER)|>
...
RiskImpactTable<-erisk_ItemProj |>
  select("RISK_ID","RISK_IDENTIFIER","P2_NUMBER.x", "PROJECT_ID",
         "PROJECT_NAME.x","P2_SUB_IDENTIFIER.x", "RISK_NAME","RISKCATEGORY", "DISCIPLINE", 
         ...
         "PERFORMANCEIMPACT_DESC", "PERFORMANCEIMPACT","PERFORMANCE_IMPACT_DESC" ,"PROJECTPHASEID", 
...
risk_transact<-left_join(risk_transact, risk_item_filt, by=c("PROJECT_ID", 
                                                             "RISK_ID","PROJECT_NAME"))     
risk_treat<-left_join(risk_treat, risk_item_filt, by=c("PROJECT_ID", "RISK_ID","PROJECT_NAME"))
...
risk_item<-filter(RiskImpactTable, RiskImpactTable$RISK_NAME == params$riskID, 
                  RiskImpactTable$P2_NUMBER.x == params$p2ID,
                  RiskImpactTable$P2_SUB_IDENTIFIER.x == params$p2sub)
risk_treatment<-filter(risk_treat, risk_treat$PROJECT_NAME == params$projID, 
                       risk_treat$RISK_NAME == params$riskID,
                       risk_treat$P2_SUB_IDENTIFIER.x == params$p2ID
                       )
...
#### Project: `r risk_item$PROJECT_NAME.x` (`r risk_item$RISK_IDENTIFIER`) SubID:`r risk_item$P2_SUB_IDENTIFIER`
...
```{r milestoneplot, fig.height=2, fig.width=9, echo=FALSE, message=FALSE}
