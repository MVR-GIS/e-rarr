...
cost_top4s<-proj_risks |>
  ...
  arrange(desc(COST_RISK_RANKING), desc(MAX_COST), desc(PROB_OCCURRENCE))|>
      dplyr::select(USACE_ORGANIZATION,PROGRAMTYPENAME,RISK_IDENTIFIER, RISK_NAME, RISKCATEGORY , MILESTONE , 
                    LIKELIHOOD, MAX_COST) |>
    dplyr::slice_head(n=4)|>
  mutate(MAX_COST = as.character(formattable::currency(MAX_COST)))|>
  kableExtra::kbl(format = "html", escape = F, 
                  col.names = c("District"," Program", "Risk ID","Risk Name","Risk Category","Project Milestone","Likelihood", "Cost Impact"), align="ccclcccc", row.names = FALSE)|>
...
schedule_top4s<-proj_risks |>
  ...
  arrange(desc(SCHEDULE_RISK_RANKING), desc(MAX_SCHED), desc(PROB_OCCURRENCE))|>
    ...
    dplyr::slice_head(n=4)|>
  kableExtra::kbl(format = "html", escape = F, 
                  col.names =c("District"," Program", "Risk ID","Risk Name","Risk Category",
