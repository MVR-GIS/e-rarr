...
output:
  html_document:
params:
  riskID: Inaccurate Floodplain for Formulation and Economic Analysis
...
risk_transact <- read_csv("C:/workspace/e-rarr//RiskItemReportShinyApp/data/RiskTransaction_FullView.csv")
...
RiskImpactTable<-erisk_ItemProj[,c(1,2,3,4,6,7,17,9,12,13,26,27,28,29,30,31,32,33,39,23,22,42,46,47,48,49,50,51,52)]
...
```{r fig.align='center', fig.height=1.75, fig.width=7, message=FALSE, warning=FALSE, milestoneplot, echo=FALSE}
...
ggplot(mockdf, aes(x=factor(ID, level=c(level_order)), y=yval))+
  ...
  geom_point(aes(x=risk_item$MILESTONE, y=0.15), colour="steelblue", fill="steelblue", shape = 21, size =8, stroke=1.5)+
...
knitr::include_graphics("C:/workspace/e-rarr/RiskItemReportShinyApp/images/montecarlo.png")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, eval = treat_condit}
risk_log<-data.frame(risk_transact)
risk_log<-risk_log %>%
  mutate('costcolor' = case_when(COST_RISK_RANKING == 0 ~ 'steelblue',
  COST_RISK_RANKING ==1 ~'green', 
  COST_RISK_RANKING == 2 ~'orange',
  COST_RISK_RANKING ==  3 ~ 'red')) %>%
  mutate('schedulecolor' = case_when(SCHEDULE_RISK_RANKING == 0 ~ 'steelblue',
  SCHEDULE_RISK_RANKING ==1 ~'green', 
  SCHEDULE_RISK_RANKING == 2 ~'orange',
  SCHEDULE_RISK_RANKING ==  3 ~ 'red')) %>%
  mutate('performancecolor' = case_when(PERFORMANCEIMPACT_RISK_RANKING == 0 ~ 'steelblue',
  PERFORMANCEIMPACT_RISK_RANKING ==1 ~'green', 
  PERFORMANCEIMPACT_RISK_RANKING == 2 ~'orange',
  PERFORMANCEIMPACT_RISK_RANKING ==  3 ~ 'red'))%>%
  mutate(TREAT_IMPLEMENT_IMG = if_else(is.na(TREATMENTS_IMPLEMENTED), "", imagepath))
         
         
risk_item_log<-filter(risk_log, RISK_NAME == params$riskID, PROJECT_NAME == params$projID)
...
`r if(!treat_condit){"Risk transactions have not been completed for this risk item"}`

```{r log table, echo=FALSE, message=FALSE, eval = treat_condit}
risk_log_table$TREAT_IMPLEMENT_IMG = sprintf("![](%s)", risk_log_table$TREAT_IMPLEMENT_IMG)
risk_log_table[1:7]%>%
kbl(format = "html", col.names =c("Edit Date", "Reason for Change", "Risk Log Note",  "Measures Implemented", "Cost","Schedule","Performance"), align="lllcccc") %>%
  ...
  column_spec (5:7,border_left = T, border_right = T)

                                                    
```

#### Risk Manager: `r risk_item$FIRST_NAME` `r risk_item$LAST_NAME`
