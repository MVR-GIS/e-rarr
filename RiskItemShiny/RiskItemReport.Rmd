...
RiskImpactTable<-erisk_ItemProj[,c(1,2,3,4,6,7,17,9,12,13,26,27,28,29,30,31,32,33,39,23,22,42,48,49,50,51,52)]
...
ggplot(mockdf, aes(x=factor(ID, level=c(level_order)), y=yval))+
  ...
  geom_point(aes(x=risk_item$MILESTONE, y=0.15), colour="steelblue3", fill="steelblue3", shape = 21, size =8, stroke=1.5)+
...
risk_log<-risk_log %>%
  mutate('costcolor' = case_when(COST_RISK_RANKING == 0 ~ 'steelblue3',
  COST_RISK_RANKING ==1 ~'green', 
  COST_RISK_RANKING == 2 ~'orange',
  COST_RISK_RANKING ==  3 ~ 'red')) %>%
  mutate('schedulecolor' = case_when(SCHEDULE_RISK_RANKING == 0 ~ 'steelblue3',
  SCHEDULE_RISK_RANKING ==1 ~'green', 
  SCHEDULE_RISK_RANKING == 2 ~'orange',
  SCHEDULE_RISK_RANKING ==  3 ~ 'red')) %>%
  mutate('performancecolor' = case_when(PERFORMANCEIMPACT_RISK_RANKING == 0 ~ 'steelblue3',
...
##Risk Manager: `r risk_item$First_Name`
